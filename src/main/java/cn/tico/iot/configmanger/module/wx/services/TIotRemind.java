...
public class TIotRemind extends Service<TIotOwner> {
	public TIotRemind(Dao dao) {
		super(dao);
	}
	...
	public List<Map> queryCountTimeOutDeviceIds(String deptid, String time) {
		...
		sql.setParam("mydept", deptid).setParam("mytime", time);
		sql.setCallback(new SqlCallback() {
			public Object invoke(Connection conn, ResultSet rs, Sql sql) throws SQLException {
				List<Map> list = new LinkedList<Map>();
				while (rs.next()) {
					NutMap map = NutMap.NEW();
					map.addv("sno", rs.getString("sno")).addv("id", rs.getString("id")).addv("time",
							rs.getString("time"));
					list.add(map);
				}
				return list;
	...
	public List<Map> queryCountPrice(String deptid) {
		String str = "select count(*) count ,sum( price) sum ,\n"
				+ "case when asset_status ='2' then \"已用\" else \"闲置\" end as asset \n" + "from t_iot_devices \n"
				+ "where 1=1 \n" + "and delflag = \"false\" \n" + "and status =\"true\"\n";
		...
		Sql sql = Sqls.create(str);
		sql.setParam("mydept", deptid);
		sql.setCallback(new SqlCallback() {
			public Object invoke(Connection conn, ResultSet rs, Sql sql) throws SQLException {
				List<Map> list = new LinkedList<Map>();
				while (rs.next()) {
					NutMap map = NutMap.NEW();
					map.addv("count", rs.getString("count")).addv("sum", rs.getString("sum")).addv("asset",
							rs.getString("asset"));
					list.add(map);
				}
				return list;
	...
	public List<Map> queryCountGroup(String deptid) {
		...
		sql.setCallback(new SqlCallback() {
			public Object invoke(Connection conn, ResultSet rs, Sql sql) throws SQLException {
				...
				while (rs.next()) {
					NutMap map = NutMap.NEW();
					map.addv("count", rs.getString("count")).addv("type", rs.getString("type"));
					list.add(map);
				}
				return list;
			}
